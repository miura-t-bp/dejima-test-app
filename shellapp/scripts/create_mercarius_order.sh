...
# 最新のMercariUS注文番号を取得
latest_mercarius_order_number=$(sh $NOW_DIR/db/get_data_from_stg.sh "SELECT mercarius_order_number FROM order_shopping_mercarius ORDER BY id DESC LIMIT 1;")
...
mercarius_order=$(sh $NOW_DIR/db/get_data_from_stg.sh "SELECT id FROM order_shopping_mercarius WHERE mercarius_order_number = '$new_mercarius_order_number';")
...
CONFIG_FILE="/etc/shellapp/app.conf"
...
echo "MercariUS注文を作成します。mercarius_order_number: $new_mercarius_order_number"
