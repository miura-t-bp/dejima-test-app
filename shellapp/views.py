...
from .utils import generate_cmd_for_create_mercarius_order, generate_cmd_for_create_bunjang_order, generate_cmd_for_create_shopee_order, generate_cmd_for_create_wechat_order, generate_cmd_for_wechat_baggage_settlement
...
def create_shopee_order_api(request):
    if request.method == 'POST':
        # リクエストのボディからJSONデータを取得
        try:
            data = json.loads(request.body)
        ...
        # Shopee注文作成用コマンドを生成
        res = generate_cmd_for_create_shopee_order(data)
